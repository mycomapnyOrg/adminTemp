// Color picker
// http://codepen.io/solaimankmail/pen/EVdmBN
var ColorPicker;
...
var global = (function(global) {
    ...
    ColorPicker = function(element, options) {
        ...
        // return an array with color and palettes from options
        this.options.palettes.unshift(this.options.color);

        this.hex = this.options.color;
        this.rgb = this.HEXtoRGB(this.hex);
        this.hsv = this.RGBtoHSV(this.rgb[0], this.rgb[1], this.rgb[2]); // Hue Saturation Value
        this.dom = {};
    ...
    ColorPicker.prototype.initPicker = function() {
        ...
        this.dom.picker.container.close = document.createElement('span');
        this.dom.picker.container.close.textContent = 'Hide';
        this.dom.picker.container.close.className = 'picker-container-close';
        this.dom.picker.container.appendChild(this.dom.picker.container.close);
        ...
        image.onload = function() {
            ...
            _this.updateCoordinates(_this.dom.picker.canvas.canvas.bounds.centerX, _this.dom.picker.canvas.canvas.bounds.centerY);
            coordinates = _this.getPositionFromColor(_this.hex);
            if (coordinates !== null) {
                _this.x = coordinates.x;
                _this.y = coordinates.y;
                _this.updateColor(_this.HEXtoRGB(_this.hex));
                _this.updateAll();
            }
            _this.options.onUpdate(_this.rgb);
        };
        ...
        this.dom.picker.canvas.canvas.addEventListener('mousedown', function(e) {
            ...
            imageData = ctx.getImageData(_this.x, _this.y, 1, 1);
            _this.updateColor(imageData.data);
            _this.hsv[2] = 1;
            _this.updateAll();
        });

        this.dom.picker.canvas.canvas.addEventListener('mousemove', function(e) { // mouse move handler
            if (dragging) {
                _this.updateCoordinates(e.clientX, e.clientY);
                imageData = ctx.getImageData(_this.x, _this.y, 1, 1);
                _this.updateColor(imageData.data);
        ...
        this.dom.picker.canvas.canvas.addEventListener('mouseup', function(e) { // click event handler
            dragging = false;
        });
        ...
        this.dom.picker.canvas.input.addEventListener('keyup', function() {
            if (this.value == _this.hex || '#' + this.value == _this.hex) {
                return;
            }
            coordinates = _this.getPositionFromColor(this.value);
            if (coordinates != null) {
                _this.x = coordinates.x;
                _this.y = coordinates.y;
                _this.updateColor(_this.HEXtoRGB(this.value));
                _this.updateAll();
            }
        });
    ...
    ColorPicker.prototype.initSlider = function() {
        ...
        this.dom.slider.slider.addEventListener('mousedown', function(e) {
            ...
            _this.dom.slider.slider.bounds = _this.dom.slider.slider.getBoundingClientRect();
            _this.dom.slider.pointer.bounds = _this.dom.slider.pointer.getBoundingClientRect();

            total = _this.updateSliderCursor(e.clientY);
            _this.updateColor(_this.HSVtoRGB(_this.hsv[0], _this.hsv[1], 1 - total));
            _this.updateAll();
        });
    ...
    ColorPicker.prototype.updateCoordinates = function(x, y) {
        ...
        var angle = Math.atan2((y - this.dom.picker.canvas.canvas.bounds.centerY), (x - this.dom.picker.canvas.canvas.bounds.centerX)); // Return angle of
    ...
    ColorPicker.prototype.updateCanvasBounds = function() {
        this.dom.picker.canvas.canvas.bounds = this.dom.picker.canvas.canvas.getBoundingClientRect(); // Canvas coordinates as to page
        this.dom.picker.canvas.pointer.bounds = this.dom.picker.canvas.pointer.getBoundingClientRect(); // Pointer coordinates as to page
        this.dom.picker.canvas.canvas.bounds.centerX = this.dom.picker.canvas.canvas.bounds.left + (this.dom.picker.canvas.canvas.bounds.width / 2); // Horizontal middle of canvas
        this.dom.picker.canvas.canvas.bounds.centerY = this.dom.picker.canvas.canvas.bounds.top + (this.dom.picker.canvas.canvas.bounds.height / 2); // Vertical middle of canvas
        this.dom.picker.canvas.canvas.bounds.radius = this.dom.picker.canvas.canvas.bounds.width / 2; // Radius of canvas
    ...
    ColorPicker.prototype.initPalettes = function() {
        ...
        this.dom.palettes.add.addEventListener('click', function() {
            addClass(_this.dom.picker.canvas.container, 'active');
            _this.updateCanvasBounds();
            palette = _this.addPalette(_this.RGBtoHEX(_this.rgb[0], _this.rgb[1], _this.rgb[2]));
            for (var i = 0; i < _this.dom.palettes.list.length; i++) {
                removeClass(_this.dom.palettes.list[i], 'active');
            }
            addClass(palette, 'active');
            _this.selectedPalette = palette;
            global.styleEditorScroll.resize();
        });
    ...
    ColorPicker.prototype.addPalette = function(color) {
        ...
        palette.addEventListener('click', function() {
            ...
            _this.selectedPalette = this;
            rgb = _this.HEXtoRGB(this.color);
            coordinates = _this.getPositionFromColor(color);
            _this.x = coordinates.x;
            _this.y = coordinates.y;
            _this.updateColor(rgb);
            _this.updateAll();
        });
...
var global = (function(global) {
    ...
    global.findParent = function(element, parentConstructor, className) {
...
global = (function(global) {
    ...
    var defaultValues = ['#34495e', '#85d6de', '#fed37f', '#fe8f8c', '#737c85', '#758594', '#ffffff', 'Varela Round', 'Open Sans'];
    ...
    var StyleEditor = function() {
        ...
        // Global variables
        global.colorPicker = this.colorPicker;
        global.overlay = this.overlay;
    };

    Object.defineProperties(StyleEditor.prototype, {
        init: {
            value: function() {
                _this = this;

                this._wrapper.classList.add('style-editor');
                this._wrapper.classList.add('close');
                this._wrapper.classList.add('elem-hide');

                // The first showing of style editor
                this._wrapper.addEventListener('animationend', function animEnd(e) {
                    _this._wrapper.classList.remove('elem-hide');
                    _this._wrapper.removeEventListener('animationend', animEnd);
                });

                this._wrapper.addEventListener('webkitAnimationEnd', function animEnd(e) {
                    _this._wrapper.classList.remove('elem-hide');
                    _this._wrapper.removeEventListener('webkitAnimationEnd', animEnd);
                });

                // Style editor toggle
                this._toggle.classList.add('style-editor__toggle');
                this._toggle.addEventListener('click', this._openEditor);

                // Color Picker
                this.colorPicker.id = 'picker';
                this.colorPicker.classList.add('picker');
                this.colorPicker.classList.add('elem-hide');

                this._styleForm.classList.add('style-editor__form');
                this._styleForm.setAttribute('autocomplete', 'off');
                this._styleForm.addEventListener('submit', function(e){
                    _this._submitForm(e, _this._makeXHRRequest);
                });

                // Heading for colors
                this._styleForm.appendChild(this._creatingHeading('h3', 'Colors'));

                // Colors
                this._styleForm.appendChild(this._creatingInputBlock('primary_color', 'Primary', '#34495e'));
                this._styleForm.appendChild(this._creatingInputBlock('highlight_color', 'Highlight', '#85d6de'));
                this._styleForm.appendChild(this._creatingInputBlock('additional_color', 'Additional', '#fed37f'));
                this._styleForm.appendChild(this._creatingInputBlock('headline_color', 'Secondary headline', '#fe8f8c'));
                this._styleForm.appendChild(this._creatingInputBlock('text_color', 'Body copy', '#737c85'));
                this._styleForm.appendChild(this._creatingInputBlock('additional_text_color', 'Additional body copy', '#758594'));
                this._styleForm.appendChild(this._creatingInputBlock('inverse_color', 'Inverse body copy', '#ffffff'));

                // Heading for fonts
                this._styleForm.appendChild(this._creatingHeading('h3', 'Fonts'));

                // Fonts
                this._styleForm.appendChild(this._creatingSelectBlock('brand_font', 'Heading', 'Varela Round'));
                this._styleForm.appendChild(this._creatingSelectBlock('regular_font', 'body copy', 'Open Sans'));
                ...
                submitButton.textContent = 'Change styles';
                submitButton.type = 'submit';
                this._styleForm.appendChild(submitButton);

                // Reset button
                var resetButton = document.createElement('button');
                resetButton.textContent = 'Reset styles';
                resetButton.type = 'button';
                resetButton.classList.add('style-editor__reset-btn');
                resetButton.addEventListener('click', function() {

                	_this._resetStyles(_this._styleForm);
                })
                this._styleForm.appendChild(resetButton);

                // Adding layout into DOM
                this._wrapper.appendChild(this._toggle);
                this._wrapper.appendChild(this.colorPicker);
                this._wrapper.appendChild(this._styleForm);

                _body.appendChild(this._wrapper);

                // Getting fonts list from Google Font Api
                this._fontListRequest('https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyDFK7jQOD7xqf9hxYMPL-fGoyNn5CRbIGQ', 'GET');

                // Initialisation of scroll for style editor
                global.styleEditorScroll = global.createScrollbar(this._styleForm, {
                    preventDefaultScroll: true
                });

                this._creatingOverlay();
            },
            enumerable: true
        },
        ...
        _openEditor: {
            value: function(e) {
                ...
                var popupButton = _body.querySelector('.popup-button');

                if (parent.classList.contains('close')) {
                    parent.classList.remove('close');
                    parent.classList.add('open');
                    if (popupButton) {
                        popupButton.style.left = parent.clientWidth + 30 + 'px';
                    }
        ...
        _creatingOverlay:{
            value: function() {
                ...
                this.overlay.classList.add('style-editor-overlay');
                this.overlay.classList.add('elem-hide');

                var overlayTextWrapper = document.createElement('div');
                overlayTextWrapper.classList.add('overlay-text-wrapper');

                var overlayHeading = document.createElement('h3');
                overlayHeading.textContent = "Hold on!";
                overlayTextWrapper.appendChild(overlayHeading);

                var overlayText = document.createElement('span');
                overlayText.textContent = 'The Server is a super busy guy, however he is trying his best to respond to your request as quickly as possible.';
                overlayTextWrapper.appendChild(overlayText);
                
                var overlayProgress = document.createElement('div');
                overlayProgress.classList.add('overlay-progress');

                var overlayProgressBar = document.createElement('div');
                overlayProgress.appendChild(overlayProgressBar);

                overlayTextWrapper.appendChild(overlayProgress);

                this.overlay.appendChild(overlayTextWrapper);
        ...
        _creatingInputBlock: {
            value: function(name, labelName, color) {
                var inputWrapper = document.createElement('div');
                inputWrapper.classList.add('style-editor__block');
                ...
                input.classList.add('style-editor__field');
                input.classList.add('style-editor__field--color');
                input.value = color;
                input.name = name;
                inputWrapper.appendChild(input);

                var colorSample = document.createElement('span');
                colorSample.classList.add('style-editor__color-sample');
                inputWrapper.appendChild(colorSample);

                return inputWrapper;
            }
        },

        _creatingSelectBlock: {
            value: function(name, labelName, standartFont) {
                var searchText = 'search';

                var selectWrapper = document.createElement('div');
                selectWrapper.classList.add('style-editor__block');
                selectWrapper.classList.add('style-editor__block--font');
                ...
                dropdown.classList.add('style-editor__font-dropdown');
                dropdown.classList.add('close');

                // dropdown toggle
                var dropdownToggle = document.createElement('button');
                dropdownToggle.type = 'button';
                dropdownToggle.textContent = 'Choose font..'
                dropdownToggle.classList.add('style-editor__font-toggle');
                dropdownToggle.addEventListener('click', this._openDropdown);
                dropdown.appendChild(dropdownToggle);

                // input for choosing font for LocalStorage
                var hiddenInput = document.createElement('input');
                hiddenInput.name = name;
                hiddenInput.value = standartFont;
                hiddenInput.classList.add('style-editor__field--font');
                hiddenInput.classList.add('style-editor__field');
                hiddenInput.classList.add('display-none');
                dropdown.appendChild(hiddenInput);

                // font list wrapper
                var fontListWrapper = document.createElement('div');
                fontListWrapper.classList.add('style-editor__font-list-wrapper');

                // font search
                var fontSearch = document.createElement('input');
                fontSearch.classList.add('style-editor__font-search');
                fontSearch.value = 'search';
                fontSearch.addEventListener('focus', function(e) {
                    e.target.value = '';
                })
                fontSearch.addEventListener('blur', function(e) {
                    e.target.value = 'search';
                })
                fontListWrapper.appendChild(fontSearch);

                // wrapper for scroll
                var scrollWrapper = document.createElement('div');
                scrollWrapper.classList.add('style-editor__scroll-wrapper');
                fontListWrapper.appendChild(scrollWrapper);

                dropdown.appendChild(fontListWrapper);
        ...
        _openDropdown: {
            value: function(e) {
                ...
                var searchInput = dropdown.querySelector('.style-editor__font-search');
        ...
        _chooseFont: {
            value: function(e, styleForm) {
                e.preventDefault();
                var target = e.target;
                if (target instanceof HTMLLIElement) {
                    var activeDropdown = styleForm.querySelector('.style-editor__font-dropdown.open');
                    var fontInput = activeDropdown.querySelector('.style-editor__field--font');
                    var dropdownToggle = activeDropdown.querySelector('.style-editor__font-toggle');
                    dropdownToggle.textContent = target.textContent;
                    fontInput.value = target.textContent;

                    var active = activeDropdown.querySelector('.active-font');
                    if (active) {
                        active.classList.remove('active-font');
                    }

                    target.classList.add('active-font');
                    activeDropdown.classList.remove('open');
                    activeDropdown.classList.add('close');
                }
            }
        },

        _creatingHeading: {
        	value: function(tag, title) {
        		// Heading for fonts
                var heading = document.createElement(tag);
                heading.textContent = title;
                heading.classList.add('style-editor__heading');
        ...
        _fontListRequest: {
            value: function(url, method) {
                ...
                // Scroll wrapper for font dropdown
                var fontWrappers = _this._styleForm.querySelectorAll('.style-editor__scroll-wrapper');
                // List of all scrollbars for font dropdowns
                var fontListScrollbars = [];
                // Array for search
                var fontArray = [];
                ...
                // Check supporting of XMLHttpRequest
                ...
                xhr.onreadystatechange = function() {
                    ...
                    if (xhr.readyState === 4) { // 4 signifies DONE
                        // req.status of 200 means success
                        if (xhr.status === 200) {
                            var fonts = JSON.parse(xhr.responseText);

                            var fontList = document.createElement('ul');
                            fontList.classList.add('style-editor__font-list');

                            // Adding fonts in font list
                            for (var i = 0, max = fonts.items.length; i < max; i++) {
                                var liElement = document.createElement('li');
                                fontArray.push(fonts.items[i].family.toLowerCase());
                                liElement.textContent = fonts.items[i].family + '';
                                fontList.appendChild(liElement);
                            }

                            for (var i = 0, max = fontWrappers.length; i < max; i++) {
                                if (i === 0) {
                                    fontWrappers[i].appendChild(fontList);
                                } else {
                                    fontWrappers[i].appendChild(fontList.cloneNode(true));
                                }

                                fontWrappers[i].addEventListener('click', function(e) {
                                    _this._chooseFont(e, _this._styleForm);
                                });


                                fontListScrollbars[i] = global.createScrollbar(fontWrappers[i], {
                                    customThumbHeight: 90,
                                    preventDefaultScroll: true
                                });

                                _this._watchSearchInput(i, fontWrappers, fontArray, fontListScrollbars[i]);
                            }


                            document.body.addEventListener('mousedown', function(e) {
                                var parent = global.findParent(e.target, HTMLUListElement, 'style-editor__font-list-wrapper');
                                var dropdown = document.querySelector('.style-editor__font-dropdown.open');
                ...
                return xhr; // return request object
            }
        },

        _watchSearchInput: {
            value: function(fontListIndex, fontWrappers, fontArray, fontListScrollbar) {
                var searchInput = fontWrappers[fontListIndex].previousElementSibling;
                var fontLiItems = (fontWrappers[fontListIndex].querySelector('.style-editor__font-list')).children;
                searchInput.addEventListener('keyup', function() {
                    var searchText = searchInput.value.toLowerCase();
                    for (var j = 0, max = fontArray.length; j < max; j++) {
                        if (fontArray[j].indexOf(searchText) === -1) {
                            fontLiItems[j].classList.add('display-none');
                        } else {
                            fontLiItems[j].classList.remove('display-none');
                        }
                    }
                    fontListScrollbar.resize();
                });
            }
        },
        ...
        // Saving inputs values in localStorage and sending them in request to 'style.php' file
        _submitForm: {
            value: function (e, makeXHRRequestFunc) {
                var version = '1.1';
                var _this = this;
                e.preventDefault();
                var data;
                var dataInputs = {};
                var fontInputs = document.querySelectorAll('.style-editor__field--font');
                var regHEX = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/;

                for (var i = 0, max = inputs.length; i < max; i++) {
                    if ( (regHEX.test(inputs[i].value) && inputs[i].classList.contains('style-editor__field--color')) ||  inputs[i].classList.contains('style-editor__field--font')) {
                ...
                for (var i = 0, max = inputs.length; i < max; i++) {
                    dataInputs[inputs[i].name] = inputs[i].value;                    
                }

                if (Storage) {
                    dataInputs.version = version;
                    localStorage.inputs = JSON.stringify(dataInputs);
                }
                ...
                for (i = 0, max = fontInputs.length; i < max; i++) {
                    WebFont.load({
                        google: {
                            families: [fontInputs[i].value]
                        }
                    });
                }
        
                start = performance.now();
                makeXHRRequestFunc('style.html', this._receive, 'POST', data, 'application/x-www-form-urlencoded');
                
        
                if (global.overlay.classList.contains('elem-hide')) {
                    global.overlay.classList.remove('elem-hide');
                    global.overlay.classList.add('overlay-show');
                }
            }
        },

        // Making XHR request and adding event listener for load event
        _makeXHRRequest:  {
            value: function (url, callback, method, postData, dataType) {
                if (!window.XMLHttpRequest) {
                    return null;
                }
        ...
        // Event handler for load event of XHR request
        _receive: {
            value: function (req) {
                var resp = req.responseText;
                var style;
                var oldStyle = _head.querySelector('[data-editor-css]');
        
                if (oldStyle) {
                    style = oldStyle;
                } else {
                    style = document.createElement('style');
                    style.setAttribute('data-editor-css', 'true')
                ...
                global.overlay.classList.remove('overlay-show');
                global.overlay.classList.add('overlay-hide');
                global.overlay.addEventListener('animationend', function animEndOverlay(e) {
                    if (e.target.classList.contains('overlay-hide')) {
                        global.overlay.classList.add('elem-hide');
                        global.overlay.classList.remove('overlay-hide');
                    }
                    global.overlay.removeEventListener('animationend', animEndOverlay);
                });
        
                global.overlay.addEventListener('webkitAnimationEnd', function webkitAnimEnd(e) {
                    if (e.target.classList.contains('overlay-hide')) {
                        global.overlay.classList.add('elem-hide');
                        global.overlay.classList.remove('overlay-hide');
                    }
                    global.overlay.removeEventListener('webkitAnimationEnd', webkitAnimEnd);
                });
        
                if (Storage) {
                    localStorage.stylesheet = resp;
                }
            }
        },

        _resetStyles: {
            value: function(styleForm) {
        
            	var oldStyle = _head.querySelector('[data-editor-css]');
            	var inputs = styleForm.querySelectorAll('.style-editor__field');
            	var colorSamples = styleForm.querySelectorAll('.style-editor__color-sample');
            	var rgbColorOfSample;
                ...
                if (oldStyle) {
                    ...
                    link.href = 'css/style.html';
                    link.rel  = 'stylesheet';
                    link.type = 'text/css';
                    _head.appendChild(link);
                    link.addEventListener("load", function(){
                       _head.removeChild(oldStyle); 
                    })
                ...
                for (var i = 0, max = inputs.length; i < max; i++) {
                	inputs[i].value = defaultValues[i];
                	if (inputs[i].classList.contains('style-editor__field--color')) {
	                	rgbColorOfSample = global.HEXtoRGB(inputs[i].value);
	        			colorSamples[i].style.background = "rgb(" + rgbColorOfSample[0] + ", " + rgbColorOfSample[1] + ", " + rgbColorOfSample[2] + ")";
	        		}
                }
            }
        }
    ...
    var inputs = document.querySelectorAll('.style-editor__field');

    // If in localStorage there are saved values for inputs, assign this values to inputs
    if (typeof savedInputs !== 'undefined') {
        for (var i = 0, max = inputs.length; i < max; i++) {
            if (inputs[i].classList.contains('style-editor__field--font')) {
                inputs[i].previousElementSibling.textContent = savedInputs[inputs[i].name] || defaultValues[i];
            } else {
                inputs[i].value = savedInputs[inputs[i].name] || defaultValues[i];
            }
        }
    }
...
var global = (function(global) {
    ...
    var colorInputs = document.querySelectorAll('.style-editor__field--color');

    var inputInFocus = colorInputs[0];
    var closeColorPiker = false;

    var colorSamples = document.querySelectorAll('.style-editor__color-sample');
    var currentColorSample = colorSamples[0];
    var rgbColorOfSample;

    var picker = new ColorPicker(global.colorPicker, {
        color: inputInFocus.value,
        onUpdate: function(rgb) {
            inputInFocus.value = picker.hex;
            currentColorSample.style.background = "rgb(" + rgb[0] + ", " + rgb[1] + ", " + rgb[2] + ")";
        }
    });

    var pickerContainer = document.getElementById('picker');
    var hideColorPickerButton = pickerContainer.querySelector('.picker-container-close');
    hideColorPickerButton.addEventListener('click', hidePicker);

    for (var i = 0, max = colorInputs.length; i < max; i++) {
        colorInputs[i].addEventListener('focus', showPicker);
        colorInputs[i].addEventListener('keyup', convertColorFromInput);
        colorSamples[i].addEventListener('click', function(e){
            e.target.previousElementSibling.focus();
        });
        rgbColorOfSample = picker.HEXtoRGB(colorInputs[i].value);
        colorSamples[i].style.background = "rgb(" + rgbColorOfSample[0] + ", " + rgbColorOfSample[1] + ", " + rgbColorOfSample[2] + ")";
    }
    ...
    function showPicker(e) {
        ...
        // checking how to open color picker
        if (target instanceof HTMLInputElement) {
            inputInFocus = target;
            currentColorSample = inputInFocus.nextElementSibling;
        } else {
            currentColorSample = target;
            inputInFocus = currentColorSample.previousElementSibling;
            var clickSample = true;
        }

        // If Color Picker close - open
        if (closeColorPiker) {
            setTimeout(function() {
                showPicker(e);
            }, 50);
        ...
        if (target.parentElement.nextElementSibling.classList.contains('picker--show')) {
        	return;
        }

        // If Color Picker open with another input
        if (pickerContainer.classList.contains('picker--show')) {
            hidePicker(e);
            showPicker(e);
            return;
        }
        ...
        // Show Color Picker
	    global.colorPicker.classList.remove('elem-hide');
	    global.colorPicker.classList.remove('picker--hide');
	    global.colorPicker.classList.add('picker--show');
	    global.colorPicker.addEventListener('animationend', function animEnd(e) {
	        if (e.target.classList.contains('picker--show')) {
	            global.styleEditorScroll.resize();
	        }

	        global.colorPicker.removeEventListener('animationend', animEnd);
	    });

	    global.colorPicker.addEventListener('webkitAnimationEnd', function webkitAnimEnd(e) {
	        if (e.target.classList.contains('picker--show')) {
	            global.styleEditorScroll.resize();
	        }

	        global.colorPicker.removeEventListener('webkitAnimationEnd', webkitAnimEnd);
	    });


        // Insert Color Picker after current input
        inputInFocus.parentElement.parentElement.insertBefore(pickerContainer, inputInFocus.parentElement.nextSibling);
        // picker.hex = target.value;

        // Update color picker coordinates
        convertColorFromInput(e);
        picker.updateCanvasBounds();
        picker.updateSliderCursor();
        picker.updatePointers();
        

        // Update Style Editor main scrollbar
        global.styleEditorScroll.resize();
        openPicker = true;
    }

    function hidePicker(e) {
        closeColorPiker = true;
        global.colorPicker.classList.remove('picker--show');
        global.colorPicker.classList.add('picker--hide');
        global.colorPicker.addEventListener('animationend', function animEnd(e) {
            if (e.target.classList.contains('picker--hide')) {
                global.colorPicker.classList.add('elem-hide');
                closeColorPiker = false;
                global.styleEditorScroll.resize();
            }
    ...
    function convertColorFromInput(e) {

        var target = e.target;
        var targetValue = target.value;
        var regHEX = /^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/;
        if (target.value == picker.hex || '#' + target.value == picker.hex) {
            return;
        }

        coordinates = picker.getPositionFromColor(target.value);
        if (coordinates != null) {
            picker.x = coordinates.x;
            picker.y = coordinates.y;
            picker.updateColor(picker.HEXtoRGB(target.value));
            picker.updateAll();
        }
    }
